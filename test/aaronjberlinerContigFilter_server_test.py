...
class aaronjberlinerContigFilterTest(unittest.TestCase):

    @classmethod
    def setUpClass(cls):
        token = environ.get('KB_AUTH_TOKEN', None)
        cls.ctx = {'token': token, 'provenance': [{'service': 'aaronjberlinerContigFilter',
        ...
        for nameval in config.items('aaronjberlinerContigFilter'):
            cls.cfg[nameval[0]] = nameval[1]
        cls.wsURL = cls.cfg['workspace-url']
        cls.wsClient = workspaceService(cls.wsURL, token=token)
        cls.serviceImpl = aaronjberlinerContigFilter(cls.cfg)
    ...
    def getWsName(self):
        ...
        wsName = "test_aaronjberlinerContigFilter_" + str(suffix)
