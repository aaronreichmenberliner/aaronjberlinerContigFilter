...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'aaronjberlinerContigFilter'):
        retconfig[nameval[0]] = nameval[1]
    return retconfig

config = get_config()

from aaronjberlinerContigFilter.aaronjberlinerContigFilterImpl import aaronjberlinerContigFilter
impl_aaronjberlinerContigFilter = aaronjberlinerContigFilter(config)
...
async_run_methods = {}
async_check_methods = {}
async_run_methods['aaronjberlinerContigFilter.filter_contigs_async'] = ['aaronjberlinerContigFilter', 'filter_contigs']
async_check_methods['aaronjberlinerContigFilter.filter_contigs_check'] = ['aaronjberlinerContigFilter', 'filter_contigs']
sync_methods['aaronjberlinerContigFilter.filter_contigs'] = True
...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or 'aaronjberlinerContigFilter'
        ...
        self.rpc_service.add(impl_aaronjberlinerContigFilter.filter_contigs,
                             name='aaronjberlinerContigFilter.filter_contigs',
                             types=[dict])
        self.method_authentication['aaronjberlinerContigFilter.filter_contigs'] = 'required'
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "aaronjberlinerContigFilter but no authentication header was passed"
                            raise err
